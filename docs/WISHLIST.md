...
- **Named-constant coverage for newer UAPI sections.** `pkg/uki` parses sections name-agnostically and round-trips any section verbatim, so `.ucode`, `.dtb`, `.dtbauto`, `.hwids`, `.efifw`, and `.splash` work today even without dedicated constants. Worth adding `SectionUcode` etc. alongside the existing `SectionLinux` / `SectionCmdline` / ... constants if/when callers want to reference them by name (e.g., to filter or inspect specific sections programmatically).

...
// Load resolves configuration from defaults, the optional YAML file, environment
// variables, and the supplied flag-override map (in that precedence order) into
// a typed Config and validates it.
//
// flagOverrides maps dotted koanf keys to their resolved typed values. The
// caller is responsible for translating from cobra/pflag flag names (which use
// dashes) to koanf keys (which use dots) and for filtering out flags that
// weren't explicitly set so flag defaults don't override file/env values.
//
// Parity with the legacy viper-based loader, locked in by internal/config/testdata/parity:
//   - Missing config file: silently uses defaults.
//   - Malformed config file: logs a warning and falls back to defaults rather

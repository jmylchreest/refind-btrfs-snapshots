...
func (m *Manager) AnalyzeBootMount(fstab *Fstab, rootFS *btrfs.Filesystem) *BootMountInfo {
	...
	for _, entry := range fstab.Entries {
		...
		// Found a /boot entry — check its filesystem type
		if entry.FSType != "btrfs" {
			// /boot is on a non-btrfs filesystem (e.g., vfat ESP)
			log.Debug().
		...
		// /boot is btrfs — check if it's the same filesystem as root
		...
		// /boot is btrfs but on a different filesystem (or we can't tell)
		log.Debug().
			Str("device", entry.Device).
	...
	// No /boot entry — it's part of the root filesystem
